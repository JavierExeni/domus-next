...
// card usada en la lista de propiedades
export const PropertyCard = ({ property, isLogged, userLogged }: Props) => {
    ...
    <div id={`${property.id}`} className="max-w-[400px] md:max-w-full md:w-full md:flex bg-white border border-gray-200 rounded-lg shadow-md m-auto h-full hover:shadow-xl transition-all duration-150">
      ...
      <div className="p-4 w-full">
        ...
        <Link
          href={
            isLogged
              ? `/propiedad/${property.id}?agent=${userLogged.id}`
...
// card usada en la lista de propiedades de un agente
export const PropertyCardByAgent = ({ property, isLogged, userLogged }: Props) => {

  const { filterBody } = usePropertiesByAgentDatosContext();
